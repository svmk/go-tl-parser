...
func Generate(schema *parser.Schema, generatedPackage string) (gnrtdStructs string, gnrtdMethods string) {
...
`
	...
	for _, interfaceInfo := range schema.InterfaceInfoes {
		interfaceInfo.Name = converter.ReplaceKeyWords(interfaceInfo.Name)
		typesCases := ""
	...
	for _, classInfoe := range schema.ClassInfoes {
		if !classInfoe.IsFunction {
			...
			structName = converter.ReplaceKeyWords(structName)
			...
			for i, prop := range classInfoe.Properties {
				...
				propName = converter.ReplaceKeyWords(propName)

				dataType, isPrimitive := converter.ConvertDataType(prop.Type, converter.ArraySideLeft, true)
				propsStrItem := ""
			...
			for i, param := range classInfoe.Properties {
				...
				propName = converter.ReplaceKeyWords(propName)
				dataType, isPrimitive := converter.ConvertDataType(param.Type, converter.ArraySideLeft, true)
			...
			if checkIsInterface(classInfoe.RootName, schema) {
				rootName := converter.ReplaceKeyWords(classInfoe.RootName)
				gnrtdStructs += fmt.Sprintf(`
			...
			methodName = converter.ReplaceKeyWords(methodName)
			returnType := strings.ToUpper(classInfoe.RootName[:1]) + classInfoe.RootName[1:]
			returnType = converter.ReplaceKeyWords(returnType)
...
func convertToArgumentName(input string) string {
	...
	paramName = converter.ReplaceKeyWords(paramName)
...
func checkIsInterface(input string, schema *parser.Schema) bool {
	for _, interfaceInfo := range schema.InterfaceInfoes {
		if interfaceInfo.Name == input || converter.ReplaceKeyWords(interfaceInfo.Name) == input {
			return true
		}
	}
