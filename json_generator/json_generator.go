...
func typeCast(typeName string) string {
	result, _ := converter.ConvertDataType(typeName, converter.ArraySideRight, false)
	if result == "int32" {
		result = "number"
	}
	return result
...
func fields(class parser.ClassInfo) (result []map[string]interface{}) {
...
func Generate(schema *parser.Schema) ([]byte, error) {
